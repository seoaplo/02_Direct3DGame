...
public:
	...
	_declspec(property(get = GetTranslationMatrix))			D3DXMATRIX& _mTranslation;
	_declspec(property(get = GetTranslationDeltaMatrix))	D3DXMATRIX& _mTranslationDelta;
	_declspec(property(get = isBeingDragged))				bool _bDrag;
	_declspec(property(put = SetTranslationRadius)) float& _fRadiusTranslation;
