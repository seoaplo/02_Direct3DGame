...
int	SCAMaterialManager::AddMaterial(INode* pNode)
{
	...
	m_MaterialList.push_back(SCAMaterial());
	SCAMaterial& Material = m_MaterialList[m_MaterialList.size() - 1];
	Material.pMaterial = pMtl;
	Material.iMaterialID = m_MaterialList.size() - 1;
	Material.szName = SCAGlobal::FixupName(pMtl->GetName());	// material 이름에 오류가 날 문자를 없애거나 바꿔야 한다.
	// sub-material
	...
	if (Material.SubMaterialNum > 0)
	{
		for (int iSub = 0; iSub < SUBMATERIAL_SIZE; iSub++)
		{
			...
			Material.SubMaterialList[iSub].szName = SCAGlobal::FixupName(pSubMtl->GetName());
...
void SCAMaterialManager::GetTexture(Mtl* pMtl, SCASubMaterial& SubMtrl)
{
...
int		SCAMaterialManager::FindMaterial(INode* pNode)
{
	Mtl* pMtl = pNode->GetMtl();
	if (pMtl)
	{
...
bool	SCAMaterialManager::ExportMaterial(FILE* pStream)
{
	if (pStream == nullptr)
	{
		return false;
...
SCAMaterialManager::SCAMaterialManager()
{
}
