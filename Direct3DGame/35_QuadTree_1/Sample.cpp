...
void Sample::DrawObject()
{
	for (int iBox = 0; iBox < NUM_OBJECTS; iBox++)
	{
		...
		if (m_pMainCamera->CheckOBBInPlane(&m_pObj->m_Box))
		{
...
void Sample::DrawSelectTreeLevel(D3DXMATRIX* pView, D3DXMATRIX* pProj)
{
	for (int iObj = 0; iObj < m_QuadTree.m_DrawObjList.size(); iObj++)
	{
		...
		m_Box.SetAmbientColor(m_pObj[iObj].m_vColor.x,
								m_pObj[iObj].m_vColor.y,
								m_pObj[iObj].m_vColor.z,
								m_pObj[iObj].m_vColor.w);
		m_Box.SetMatrix(&m_pObj[iObj].m_matWorld, &m_pMainCamera->_matView, &m_pMainCamera->_matProj);
		m_Box.Render(GetContext());
	}
}
...
void Sample::DrawMiniMap()
{
	...
	if (m_MiniMap.BeginRender(GetContext()))
	{
		...
		m_pMainCamera->SetMatrix(NULL, &m_MiniMap._matView, &m_MiniMap._matProj);
