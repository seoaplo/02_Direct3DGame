...
void SOAExporter::Convert()
{
	...
	for (int iObject = 0; iObject < g_iNodeMaxNum; iObject++)
	{
		INode* pNode = SAExporter::g_NodeList.find(iObject)->second;
		int iMaterialNum = SAExporter::FindMaterial(pNode);
		m_SOAManager.AddObject(pNode, g_Scene, g_Interval, iObject, iMaterialNum);
	}
}
