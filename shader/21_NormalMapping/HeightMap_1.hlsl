...
//		   normal = normalize(mul(normal, tanMat));
//
//	float  fDot = saturate(dot(normal.xyz, -input.vLightDir));
//	float3 LightColor = cb_DiffuseLightColor.rgb * fDot;
//	
//	// 스펙큘러 조명 
//	//float3 vLightDir = normalize(cb_vLightVector.xyz - input.vWorldPos.xyz);
...
//		int i1 = (iCount + 1) % 3;
//		int i2 = (iCount + 2) % 3;
//		vTangent = float4(CreateTangent(input[i0].vPos, input[i1].vPos, input[i2].vPos,
...
//
//		float3 vLightDir = normalize(cb_vLightVector.xyz - input[iCount].vWorldPos);
