...
struct ParentSort
{
	bool operator()(SMesh& First, SMesh& Second)
	{
		if (_tcscmp(First.ParentName, Second.name) != 0)
		{
			return false;
		}
		return  true;
...
struct RootSort
{
	bool operator()(SMesh& First, SMesh& Second)
	{
		if (_tcscmp(First.ParentName, L"none") == 0)
		{
			if (_tcscmp(Second.ParentName, L"none") != 0)
			{
				return true;
			}
		}
