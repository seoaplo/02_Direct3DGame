...
bool Sample::Init()
{
	bool bRead = ExportData(_T("../../testData/3DMax/Test1.SSS"));
	//--------------------------------------------------------------------------------------
	// 월드  행렬
	//--------------------------------------------------------------------------------------
	...
	m_Model.Create(GetDevice(), GetContext(), L"Box001", _T("../../testData/3DMax/Test1.SSS"), _T("Test.hlsl"));
...
bool Sample::Render()
{
	...
	m_Model.SetMatrix(&m_matWorld, &m_pMainCamera->_matView, &m_pMainCamera->_matProj);

	m_Model.Render(GetContext());
