#include "SFrustum.h"
bool S_PLANE::CreatePlane(D3DXVECTOR3 v0, D3DXVECTOR3 v1, D3DXVECTOR3 v2)
{
...
bool S_PLANE::CreatePlane(D3DXVECTOR3 vNormal, D3DXVECTOR3 v0)
{
	D3DXVec3Normalize(&vNormal, &vNormal);
...
void SFrustum::ExtractPlanesD3D(S_PLANE* pPlanes, const D3DXMATRIX & comboMatrix)
{
...
void SFrustum::SetMatrix(D3DXMATRIX* pWorld, D3DXMATRIX* pView, D3DXMATRIX* pProj)
{
	if (pWorld != NULL)
	{
		m_matWorld = *pWorld;
...
bool SFrustum::CreateRenderBox(ID3D11Device*  pDevice, ID3D11DeviceContext* pContext)
{
	...
	// vertex color
	m_pPixelShader.Attach(DXGame::LoadPixelShaderFile(pDevice, L"../../shader/Shape/Box.hlsl", "PS_Color"));
...
bool SFrustum::Render(ID3D11DeviceContext*    pContext)
{
	PreRender(pContext);
...
bool SFrustum::PreRender(ID3D11DeviceContext*    pContext)
{
	...
	// 뒷면
	m_VertexList[6] = PNCT_VERTEX(m_vFrustum[6], D3DXVECTOR3(0.0f, 0.0f, 1.0f), D3DXVECTOR4(0.0f, 1.0f, 0.0f, 0.5f), D3DXVECTOR2(0.0f, 0.0f));
	m_VertexList[5] = PNCT_VERTEX(m_vFrustum[5], D3DXVECTOR3(0.0f, 0.0f, 1.0f), D3DXVECTOR4(0.0f, 1.0f, 0.0f, 0.5f), D3DXVECTOR2(1.0f, 0.0f));
	m_VertexList[4] = PNCT_VERTEX(m_vFrustum[4], D3DXVECTOR3(0.0f, 0.0f, 1.0f), D3DXVECTOR4(0.0f, 1.0f, 0.0f, 0.5f), D3DXVECTOR2(1.0f, 1.0f));
	...
	pContext->UpdateSubresource(m_pBoxShape->_dxobj.g_pVertexBuffer.Get(), 0, NULL, m_VertexList.data(), 0, 0);
	m_pBoxShape->SetMatrix(NULL, &m_matView, &m_matProj);
	m_pBoxShape->_dxobj.PreRender(pContext);
	return true;
}

bool SFrustum::PostRender(ID3D11DeviceContext*    pContext)
{
	if (m_pBoxShape == nullptr) return true;
 	pContext->UpdateSubresource(m_pBoxShape->_dxobj.g_pConstantBuffer.Get(), 0, NULL, &m_pBoxShape->_cbData, 0, 0);
	m_pBoxShape->_dxobj.PostRender(pContext);
	return true;
}
bool SFrustum::Release()
{
	SAFE_DEL(m_pBoxShape);
	return true;
}
bool SFrustum::ResetDevice()
{
	return true;
}
...
bool SFrustum::ClassifyPoint(D3DXVECTOR3* pv)
{
	float		fPlaneToPoint;
	for (int iPlane = 0; iPlane < 6; iPlane++)
	{
...
// 구 제외 처리
bool SFrustum::ClassifySphere(S_SPHERE* pSphere)//D3DXVECTOR3* pv, float radius )
{
