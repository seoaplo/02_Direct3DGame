...
cbuffer cb0 : register(b0)
{
	...
	float fOutline : packoffset(c4.y);
...
float4 PS(PS_INPUT vIn) : SV_Target
{
	...
	{
		float fAlpha = fCicleDistance - fOutline;
		if (fAlpha < 0.0f) fAlpha = 1.0f;
		else fAlpha = (fDistance - fOutline - fAlpha) / (fDistance - fOutline);
		float4 SourceColor = vIn.vColor;
		RenderColor = float4(((1 - fAlpha) *  RenderColor.xyz), RenderColor.w)
		+ float4((fAlpha *  SourceColor.xyz), fAlpha);
