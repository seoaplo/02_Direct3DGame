...
bool Sample::Init()
{
	...
	// Direction Setting
	if (m_Direction.Create(m_pDevice, m_pImmediateContext) == false)
	{
		return false;
	}
	...
	if (m_Plane.Create(GetDevice(), m_pImmediateContext, L"../../data/Image/Attacker3.bmp") == false)
	{
		return false;
	}
	// Box Setting
	...
	if (m_Box.Create(GetDevice(), m_pImmediateContext, L"../../data/Image/checker_with_numbers.bmp") == false)
	{
		return false;
	}
	...
	D3DXMatrixIdentity(&m_matWorldPlaneBase);
	...
	float fAspecRatio = m_nWindowWidth / (float)m_nWindowHeight;
	...
	m_MainCamera.SetProjMatrix(D3DX_PI * 0.25,
		...
		1.0f,
		100.0f);
	m_MainCamera.SetWindow(m_nWindowWidth, m_nWindowHeight);
...
bool Sample::Frame()
{
	...
	//	m_MainCamera.SetPosition(m_Camera[m_iCameraType].GetPositionVector());
	//	m_MainCamera.SetLook(m_Camera[m_iCameraType].GetLookVector());
	//}
	//--------------------------------------------------------------------------------------
	// 카메라 포지션 전후 좌우 이동
	//--------------------------------------------------------------------------------------
	...
	{
		m_vObjectPosition.x += I_Timer.GetSPF() * 2.0f;
	}
	...
	m_MainCamera.Update(I_Timer.GetSPF());

	m_World[0] = m_MainCamera.GetWorldMatrix();
	m_World[1] = m_matWorldPlaneBase * m_World[0];
