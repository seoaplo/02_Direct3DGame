...
int SkinFileLoader::Load(T_STR FilePath)
{
	int Key = -1;
	...
	for (int iMesh = 0; iMesh < SceneData.iNumObjects; iMesh++)
	{
		...
		_stscanf_s(m_Paser.m_pBuffer, _T("\n%s %s %d %d"),
			...
			&iSubMeshSize);
		Key = I_SkinObjectManager.GetSize() - 1;
		...
		memcpy(&pSkinObject->m_InvMatrixList, m_InvMatrix, sizeof(D3DXMATRIX) * AnimationMatrixList);
		...
		pSkinObject->Init();
		pSkinObject->Create(m_pDevice, m_pContext, L"Character.hlsl");
	}
